<?php

namespace Mdanter\Ecc\Tests;

use Mdanter\Ecc\EccFactory;
use Mdanter\Ecc\Math\BcMath;
use Mdanter\Ecc\Math\Gmp;
use Mdanter\Ecc\MathAdapter;
use Mdanter\Ecc\PublicKey;
use Mdanter\Ecc\Signature;
use Mdanter\Ecc\EcDH;

class NistCurveTest extends AbstractTestCase
{

    /**
     *
     * @dataProvider getAdapters
     */
    public function testP192CurveAnsiX962ValidityTest(MathAdapter $math)
    {
        $generator = EccFactory::getNistCurves($math)->generator192();

        $d = '651056770906015076056810763456358567190100156695615665659';
        $Q = $generator->mul($d);

        $this->assertEquals($math->hexDec('0x62B12D60690CDCF330BABAB6E69763B471F994DD702D16A5'), $Q->getX());

        $k = '6140507067065001063065065565667405560006161556565665656654';
        $R = $generator->mul($k);
        $expected = EccFactory::getNistCurves($math)->curve192()
            ->getPoint($math->hexDec('0x885052380FF147B734C330C43D39B2C4A89F29B0F749FEAD'),
                $math->hexDec('0x9CF9FA1CBEFEFB917747A3BB29C072B9289C2547884FD835'));

        $this->assertTrue($expected->equals($R));

        $u1 = '2563697409189434185194736134579731015366492496392189760599';
        $u2 = '6266643813348617967186477710235785849136406323338782220568';

        $temp = $generator->mul($u1)->add($Q->mul($u2));

        $this->assertEquals($math->hexDec('0x885052380FF147B734C330C43D39B2C4A89F29B0F749FEAD'), $temp->getX());
        $this->assertEquals($math->hexDec('0x9CF9FA1CBEFEFB917747A3BB29C072B9289C2547884FD835'), $temp->getY());
    }

    public function getB22Params()
    {
        return $this->_getAdapters([
            [ 'cd6d0f029a023e9aaca429615b8f577abee685d8257cc83a', '0x00019c410987680e9fb6c0b6ecc01d9a2647c8bae27721bacdfc', false ],
            [ '00017f2fce203639e9eaf9fb50b81fc32776b30e3b02af16c73b', '0x95da95c5e72dd48e229d4748d4eee658a9a54111b23b2adb', false ],
            [ '0x4f77f8bc7fccbadd5760f4938746d5f253ee2168c1cf2792', '0x000147156ff824d131629739817edb197717c41aab5c2a70f0f6', false ],
            [ '0xc58d61f88d905293bcd4cd0080bcb1b7f811f2ffa41979f6', '0x8804dc7a7c4c7f8b5d437f5156f3312ca7d6de8a0e11867f', true ],
            [ '0xcdf56c1aa3d8afc53c521adf3ffb96734a6a630a4a5b5a70', '0x97c1c44a5fb229007b5ec5d25f7413d170068ffd023caa4e', true ],
            [ '0x89009c0dc361c81e99280c8e91df578df88cdf4b0cdedced', '0x27be44a529b7513e727251f128b34262a0fd4d8ec82377b9', true ],
            [ '0x6a223d00bd22c52833409a163e057e5b5da1def2a197dd15', '0x7b482604199367f1f303f9ef627f922f97023e90eae08abf', true ],
            [ '0x6dccbde75c0948c98dab32ea0bc59fe125cf0fb1a3798eda', '0x0001171a3e0fa60cf3096f4e116b556198de430e1fbd330c8835', false ],
            [ '0xd266b39e1f491fc4acbbbc7d098430931cfa66d55015af12', '0x193782eb909e391a3148b7764e6b234aa94e48d30a16dbb2', false ],
            [ '0x9d6ddbcd439baa0c6b80a654091680e462a7d1d3f1ffeb43', '0x6ad8efc4d133ccf167c44eb4691c80abffb9f82b932b8caa', false ],
            [ '0x146479d944e6bda87e5b35818aa666a4c998a71f4e95edbc', '0xa86d6fe62bc8fbd88139693f842635f687f132255858e7f6', false ],
            [ '0xe594d4a598046f3598243f50fd2c7bd7d380edb055802253', '0x509014c0c4d6b536e3ca750ec09066af39b4c8616a53a923', false ]
        ]);
    }

    /**
     *
     * @dataProvider getB22Params
     */
    public function testP192CurveEcdsavsB22PointValidityTest(MathAdapter $math, $x, $y, $expected)
    {
        $x = $math->hexDec($x);
        $y = $math->hexDec($y);

        $generator = EccFactory::getNistCurves($math)->generator192();

        $this->assertEquals($expected, $generator->isValid($x, $y));
    }

    /**
     *
     * @dataProvider getAdapters
     */
    public function testSignatureValidity(MathAdapter $math)
    {
        $generator = EccFactory::getNistCurves($math)->generator192();
        $d = '651056770906015076056810763456358567190100156695615665659';
        $k = '6140507067065001063065065565667405560006161556565665656654';
        $e = '968236873715988614170569073515315707566766479517';

        $publicKey = $generator->getPublicKey($generator->mul($d)->getX(), $generator->mul($d)->getY());
        $privateKey = $publicKey->getPrivateKey($d);

        $sig = $privateKey->sign($e, $k);

        $this->assertEquals('3342403536405981729393488334694600415596881826869351677613', $sig->getR());
        $this->assertEquals('5735822328888155254683894997897571951568553642892029982342', $sig->getS());
        $this->assertTrue($publicKey->verifies($e, $sig));
        $this->assertFalse($publicKey->verifies($math->sub($e, 1), $sig));
    }

    /**
     *
     * @dataProvider getAdapters
     */
    public function testSignatureValidityWithGeneratedKeys(MathAdapter $math)
    {
        $generator = EccFactory::getNistCurves($math)->generator192();
        $curve = EccFactory::getNistCurves($math)->curve192();
        $n = $generator->getOrder();
        $secret = $math->rand($n);
        $secretG = $generator->mul($secret);
        $hash = $math->rand($n);

        $publicKey = $generator->getPublicKey($secretG->getX(), $secretG->getY());
        $privateKey = $publicKey->getPrivateKey($secret);

        $signature = $privateKey->sign($hash, $math->rand($n));

        $this->assertTrue($publicKey->verifies($hash, $signature), 'Correctly validates valid hash.');
        $this->assertFalse($publicKey->verifies($math->sub($hash, 1), $signature), 'Correctly rejects tampered hash.');
    }

    public function getB24Params()
    {
        return $this->_getAdapters(
            [
                [
                    '',
                    '0xd9dbfb332aa8e5ff091e8ce535857c37c73f6250ffb2e7ac',
                    '0x282102e364feded3ad15ddf968f88d8321aa268dd483ebc4',
                    '0x64dca58a20787c488d11d6dd96313f1b766f2d8efe122916',
                    '0x1ecba28141e84ab4ecad92f56720e2cc83eb3d22dec72479',
                    true
                ],
                [
                    '',
                    '0x3e53ef8d3112af3285c0e74842090712cd324832d4277ae7',
                    '0xcc75f8952d30aec2cbb719fc6aa9934590b5d0ff5a83adb7',
                    '0x8285261607283ba18f335026130bab31840dcfd9c3e555af',
                    '0x356d89e1b04541afc9704a45e9c535ce4a50929e33d7e06c',
                    true
                ],
                [
                    '',
                    '0x16335dbe95f8e8254a4e04575d736befb258b8657f773cb7',
                    '0x421b13379c59bc9dce38a1099ca79bbd06d647c7f6242336',
                    '0x4141bd5d64ea36c5b0bd21ef28c02da216ed9d04522b1e91',
                    '0x159a6aa852bcc579e821b7bb0994c0861fb08280c38daa09',
                    false
                ],
                [
                    '',
                    '0xfd14cdf1607f5efb7b1793037b15bdf4baa6f7c16341ab0b',
                    '0x83fa0795cc6c4795b9016dac928fd6bac32f3229a96312c4',
                    '0x8dfdb832951e0167c5d762a473c0416c5c15bc1195667dc1',
                    '0x1720288a2dc13fa1ec78f763f8fe2ff7354a7e6fdde44520',
                    false
                ],
                [
                    '',
                    '0x674f941dc1a1f8b763c9334d726172d527b90ca324db8828',
                    '0x65adfa32e8b236cb33a3e84cf59bfb9417ae7e8ede57a7ff',
                    '0x9508b9fdd7daf0d8126f9e2bc5a35e4c6d800b5b804d7796',
                    '0x36f2bf6b21b987c77b53bb801b3435a577e3d493744bfab0',
                    false
                ],
                [
                    '',
                    '0x10ecca1aad7220b56a62008b35170bfd5e35885c4014a19f',
                    '0x04eb61984c6c12ade3bc47f3c629ece7aa0a033b9948d686',
                    '0x82bfa4e82c0dfe9274169b86694e76ce993fd83b5c60f325',
                    '0xa97685676c59a65dbde002fe9d613431fb183e8006d05633',
                    false
                ],
                [
                    '',
                    '0x6636653cb5b894ca65c448277b29da3ad101c4c2300f7c04',
                    '0xfdf1cbb3fc3fd6a4f890b59e554544175fa77dbdbeb656c1',
                    '0xeac2ddecddfb79931a9c3d49c08de0645c783a24cb365e1c',
                    '0x3549fee3cfa7e5f93bc47d92d8ba100e881a2a93c22f8d50',
                    false
                ],
                [
                    '',
                    '0xa82bd718d01d354001148cd5f69b9ebf38ff6f21898f8aaa',
                    '0xe67ceede07fc2ebfafd62462a51e4b6c6b3d5b537b7caf3e',
                    '0x4d292486c620c3de20856e57d3bb72fcde4a73ad26376955',
                    '0xa85289591a6081d5728825520e62ff1c64f94235c04c7f95',
                    false
                ],
                [
                    '',
                    '0x7d3b016b57758b160c4fca73d48df07ae3b6b30225126c2f',
                    '0x4af3790d9775742bde46f8da876711be1b65244b2b39e7ec',
                    '0x95f778f5f656511a5ab49a5d69ddd0929563c29cbc3a9e62',
                    '0x75c87fc358c251b4c83d2dd979faad496b539f9f2ee7a289',
                    false
                ],
                [
                    '',
                    '0x9362f28c4ef96453d8a2f849f21e881cd7566887da8beb4a',
                    '0xe64d26d8d74c48a024ae85d982ee74cd16046f4ee5333905',
                    '0xf3923476a296c88287e8de914b0b324ad5a963319a4fe73b',
                    '0xf0baeed7624ed00d15244d8ba2aede085517dbdec8ac65f5',
                    true
                ],
                [
                    '',
                    '0xcc6fc032a846aaac25533eb033522824f94e670fa997ecef',
                    '0xe25463ef77a029eccda8b294fd63dd694e38d223d30862f1',
                    '0x066b1d07f3a40e679b620eda7f550842a35c18b80c5ebe06',
                    '0xa0b0fb201e8f2df65e2c4508ef303bdc90d934016f16b2dc',
                    false
                ],
                [
                    '',
                    '0x955c908fe900a996f7e2089bee2f6376830f76a19135e753',
                    '0xba0c42a91d3847de4a592a46dc3fdaf45a7cc709b90de520',
                    '0x1f58ad77fc04c782815a1405b0925e72095d906cbf52a668',
                    '0xf2e93758b3af75edf784f05a6761c9b9a6043c66b845b599',
                    false
                ],
                [
                    '',
                    '0x31f7fa05576d78a949b24812d4383107a9a45bb5fccdd835',
                    '0x8dc0eb65994a90f02b5e19bd18b32d61150746c09107e76b',
                    '0xbe26d59e4e883dde7c286614a767b31e49ad88789d3a78ff',
                    '0x8762ca831c1ce42df77893c9b03119428e7a9b819b619068',
                    false
                ],
                [
                    '',
                    '0x66aa8edbbdb5cf8e28ceb51b5bda891cae2df84819fe25c0',
                    '0x0c6bc2f69030a7ce58d4a00e3b3349844784a13b8936f8da',
                    '0xa4661e69b1734f4a71b788410a464b71e7ffe42334484f23',
                    '0x738421cf5e049159d69c57a915143e226cac8355e149afe9',
                    false
                ],
                [
                    '',
                    '0xbcfacf45139b6f5f690a4c35a5fffa498794136a2353fc77',
                    '0x6f4a6c906316a6afc6d98fe1f0399d056f128fe0270b0f22',
                    '0x9db679a3dafe48f7ccad122933acfe9da0970b71c94c21c1',
                    '0x984c2db99827576c0a41a5da41e07d8cc768bc82f18c9da9',
                    false
                ]
            ]);
    }

    /**
     *
     * @dataProvider getB24Params
     */
    public function testP192CurveEcdsavsB24SignatureValidityTest(MathAdapter $math, $msg, $Qx, $Qy, $R, $S, $expected)
    {
        $msg = $math->hexDec($msg);
        $Qx = $math->hexDec($Qx);
        $Qy = $math->hexDec($Qy);
        $R = $math->hexDec($R);
        $S = $math->hexDec($S);

        $generator = EccFactory::getNistCurves($math)->generator192();
        $curve = EccFactory::getNistCurves($math)->curve192();
        $publicKey = $generator->getPublicKey($Qx, $Qy);

        $actual = $publicKey->verifies($math->digestInteger($msg), new Signature($R, $S));

        $this->assertEquals($expected, $actual);
    }

    /**
     *
     * @dataProvider getAdapters
     */
    public function testDiffieHellman(MathAdapter $math)
    {
        $generator = EccFactory::getNistCurves($math)->generator192();
        $alice = $generator->createKeyExchange();
        $bob = $generator->createKeyExchange();

        $alice->exchangeKeys($bob);
        $this->assertTrue($alice->calculateKey() == $bob->calculateKey());

        $unencrypted = 'This is some text';

        $encrypted = $alice->encrypt($unencrypted);
        $this->assertEquals($unencrypted, $bob->decrypt($encrypted));

        $encrypted = $bob->encrypt($unencrypted);
        $this->assertEquals($unencrypted, $alice->decrypt($encrypted));
    }
}
